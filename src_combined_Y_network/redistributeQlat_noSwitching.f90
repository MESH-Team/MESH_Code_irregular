...
subroutine mesh_diffusive_forward(dtini_given, t0, t, tfin, saveInterval,j)
    ...
    real :: a1, a2, a3, a4, b1, b2, b3, b4, dd1, dd2, dd3, dd4, h1, h2, h3, h4, xt, allqlat, allqlat_us
...
! Note: lateralFlow(1,j) is already added to the boundary
        ...
        do k=1,ndep(j)
            kk=uslinks(k,j)
            allqlat_us=allqlat_us+(sum(lateralFlow(2:nx1(kk),kk) * dx(1:nx1(kk)-1,kk)))*0.9
        end do

        !if (allqlat_us .ne. 0.) then
        !    print*, j, ndep(j), allqlat_us
        !    pause
        !end if

        !print*,j, ndep(j), allqlat, allqlat_us, lateralFlow(2:nx1(2),2); pause 20002

        allqlat = (sum(lateralFlow(2:ncomp,j) * dx(1:ncomp-1,j)))*0.1+allqlat_us
