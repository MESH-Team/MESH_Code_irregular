subroutine matrixp(j)
    ...
    do i=1,ncomp
        ...
        st(2,2)=-2*f*oldQ(i,j)*grav*area(i)/co(i)/co(i)
...
!           Nazmul: WHAT IS di? Need to check how to change it for natural channel
    ...
    do i=2,ncomp-1
        ...
        elseif(i.eq.2.or.i.eq.(ncomp-1)) then
            ...
            d2(i)=eps2(i)*eia*(oldQ(i+1,j)-oldQ(i,j))
            ...
            d2(i)=eps2(i)*eia*(oldQ(i+1,j)-oldQ(i,j))-eps4(i)*(oldQ(i+2,j)-3*oldQ(i+1,j)+3*oldQ(i,j)-oldQ(i-1,j))
        endif
        ! print *, i,d1(i),d2(i),eps2(i),area(i+1),area(i)
    end do
