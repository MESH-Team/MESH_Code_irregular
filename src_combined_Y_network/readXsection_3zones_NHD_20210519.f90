...
! the top width calculation needs to check
    ...
    real compoundSKK(nel), compoundMann, el_min_1, elev(nel), addedDepth, highResBfd
    real el_incr_1, el_incr_2, el_incr_3, elevSpcl_1, elevSpcl_2
    integer i1, i2
...
400   close(11)
      ...
      allYcs(3,1)=allYcs(3,1)+highResBfd

      allYcs(4,3)=allYcs(4,3)+highResBfd
      ...
      !el_incr=el_range/real(nel-6)

      ! change 20210519
      elevSpcl_1 = allYcs(2,2)-highResBfd
      elevSpcl_2 = allYcs(2,2)+highResBfd

      el_incr_1=(elevSpcl_1-(el_min + addedDepth))/(real(nel-16)/2)

      !print*, elevSpcl_1, elevSpcl_2,el_incr_1,(el_min + addedDepth), (nel-16)/2; pause 1001
      ...
      el_incr_1 = (elev(300) - elev(5)) / (300-5)

      do kkk = 6, 300
        elev(kkk) = elev(kkk-1) + el_incr_1
      end do

      !print*, elev(6),elevSpcl_1,elevSpcl_2,el_incr_1,elev(300)  ; pause

      el_incr_2 = (elev(400) - elev(300)) / (400-300)

      do kkk = 301, 400
        elev(kkk) = elev(kkk-1) + el_incr_2
      end do



      el_incr_3 = (((el_max-el_min)*timesDepth+el_min) - elev(400)) / (501-400)

      !print*, (el_max-el_min)*timesDepth+el_min, elev(400), el_incr_3; pause

      do kkk = 401, 501
        elev(kkk) = elev(kkk-1) + el_incr_3
      end do
