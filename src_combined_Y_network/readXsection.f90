subroutine readXsection(k,rmanning,timesDepth,num_reach)
...
!     output cs data
      ...
      do j=1,nel
        ...
        if (diffArea(j) .eq. 0) diffArea(j) = TOLERANCE
...
10      format(f9.2,3f12.2,2f20.3,2f16.3)
        ...
        xsec_tab(1,j,k,num_reach) = el1(j)
        xsec_tab(2,j,k,num_reach) = a1(j)
        xsec_tab(3,j,k,num_reach) = peri1(j)
        xsec_tab(4,j,k,num_reach) = redi1(j)
        xsec_tab(5,j,k,num_reach) = conv1(j)
        xsec_tab(6,j,k,num_reach) = tpW1(j)
        xsec_tab(7,j,k,num_reach) = newI1(j)
