    ...
    !!START+++++++ If the channel has boundary originated from a junction+++++++
      ...
      !!END+++++++ If the channel has boundary originated from a junction+++++++
        ...
        if (S_0 .gt. 0.) then
            ...
            call normal_crit_y(j,oldY(1,j), q_sk_multi, S_0, currentQ, y_norm_us, y_crit_us, area_norm, area_crit)
        end if
        ...
        if (t .lt. 30) then
            higherLimitCount(1) = 0; lowerLimitCount(1) = ncomp         ! diffusive
            higherLimitCount(2) = 0; lowerLimitCount(2) = ncomp         ! diffusive
            higherLimitCount(3) = 0; lowerLimitCount(3) = ncomp
        else
            higherLimitCount(1) = 0; lowerLimitCount(1) = ncomp         ! dynamic
            higherLimitCount(2) = 0; lowerLimitCount(2) = ncomp         ! dynamic
            higherLimitCount(3) = 0; lowerLimitCount(3) = ncomp         ! dynamic
        end if
