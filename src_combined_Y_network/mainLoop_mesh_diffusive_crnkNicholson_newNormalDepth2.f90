...
!!++++++++++++++++++++ Diffusive wave Backward sweep starts +++++++++++++++++++!!
        ...
        if (frds .ge. 1.0 ) print*, j, 'normal'

       ! if ((normalDepth(j) .eq. 1) .and. (j .eq. nlinks)) newY(ncomp,j) = y_norm_ds
    ...
    !     interpolate the cross section attributes based on water elevation
            ...
            if (i .gt. 1) then
                ...
                !if (j .eq. 4) print*, 'In diffusive backward', i, i-1, j, q_sk_multi, slope, qp(i-1,j)

                !if ( normalDepth(j) .eq. 1) then        ! for Muskingum Cunge routing: changing all WL as normal depth
                if (dimensionless_D(i-1,j) .lt. 1) then
                    ...
                    !print*, 'here', j, i
                    !if (j .ge. 5) then
                    print*,j,i-1 !, newY(i,j), sfi, qp(i,j),co(i), dx(i-1,j)
